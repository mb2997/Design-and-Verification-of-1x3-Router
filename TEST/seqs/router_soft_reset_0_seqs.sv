`ifndef ROUTER_SOFT_RESET_0_SEQS
`define ROUTER_SOFT_RESET_0_SEQS

class router_soft_reset_0_seqs extends router_rd_seqs;

    //Factory registration
    `uvm_object_utils(router_soft_reset_0_seqs)
    ...
    //New constructor
    function new(string name = "router_soft_reset_0_seqs");
        super.new(name);
    endfunction
    ...
    task body();    
        ...
        begin
            ...
            $display("\n--------------------------------- READ TRANSACTION (SOFT_RESET_0) : %0d ---------------------------------\n",trans_no);
            ...
            `uvm_info(get_type_name(),$sformatf("SOFT_RESET_0 QUEUE Array = \n%p",soft_reset_0_q),UVM_MEDIUM)

            if(soft_reset_0_q.size() == 0)
                `uvm_error("TEST_FAIL","SOFT_RESET_0 is not asserted from DUT")
            else
                `uvm_info("TEST_PASS","SOFT_RESET_0 is asserted from DUT",UVM_MEDIUM)

        end

    endtask : body

    task catch_soft_reset_0_val();

        for(int i=1; i>0; i++)
        begin
            void'(uvm_hdl_read("router_top.ROUTER_DUT.SYNCHRONIZER.soft_reset_0",soft_reset_0));
            wait_for_negedge_clock(1);
            if(soft_reset_0 == 1)
            begin
                soft_reset_0_q.push_back(soft_reset_0);
            end
        end
    endtask

endclass : router_soft_reset_0_seqs
