`ifndef ROUTER_MIN_HEADER_LEN_SEQS
`define ROUTER_MIN_HEADER_LEN_SEQS

class router_min_header_len_seqs extends router_wr_seqs;

    //Factory registration
    `uvm_object_utils(router_min_header_len_seqs)

    //New constructor
    function new(string name = "router_min_header_len_seqs");
        super.new(name);
    endfunction
    ...
    task body();    
        ...
        repeat(no_of_trans)
        begin
            assert(wr_trans_h.randomize() with {header_len == 1;});
...
endclass : router_min_header_len_seqs
